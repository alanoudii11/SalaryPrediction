...
data_clustring <- subset(sampleDataset, select = -num_salary)
View(data_clustring)
...
```{r} 
data_clustring$workclass <-  as.numeric(factor(data_clustring$workclass))
data_clustring$fnlwgt <-  as.numeric(factor(data_clustring$fnlwgt))
data_clustring$age <-  as.numeric(factor(data_clustring$age))
data_clustring$education.num <-  as.numeric(factor(data_clustring$education.num))
data_clustring$marital.status <-  as.numeric(factor(data_clustring$marital.status))
data_clustring$occupation <-  as.numeric(factor(data_clustring$occupation))
data_clustring$relationship <-  as.numeric(factor(data_clustring$relationship))
data_clustring$race <-  as.numeric(factor(data_clustring$race))
data_clustring$hours.per.week <-  as.numeric(factor(data_clustring$hours.per.week))
data_clustring$sex <-  as.numeric(factor(data_clustring$sex))
data_clustring$native.country <-  as.numeric(factor(data_clustring$native.country))
data_clustring <- scale(data_clustring[, sapply(data_clustring, is.numeric)])

```


#Data after preparing and converting categorical attributes into numeric values
...
Total Within-Cluster Sum of Squares: 95967.99
Precision: 0.637
Recall: 0.7446
...
Total Within-Cluster Sum of Squares: 86496.88
Precision: 0.6753
Recall: 0.4412
...
Total Within-Cluster Sum of Squares: 78869.05
Precision: 0.6730
Recall: 0.3937
...
Clustering Result 1: kmeans_model$tot.withinss = 95967.99

The clusters appear to be well-defined and separated in the graph.
The WCSS value of 95967.99 is moderate, suggesting that the clusters are neither overly compact nor overly spread out.

Clustering Result 2: kmeans_model$tot.withinss = 86496.88

The clusters appear to be compact and well-separated in the graph.
The WCSS value of 86496.88 is lower than the first clustering result, indicating that the clusters are more compact in this case.

Clustering Result 3: kmeans_model$tot.withinss = 78869.05

The clusters appear to be very compact and well-separated in the graph.
The WCSS value of 78869.05 is the lowest of the three clustering results, indicating that the clusters are the most compact in this case.


Recommendation:

Based on the WCSS values and the visual inspection of the clusters in the graphs, we would suggest that Clustering Result 3 (kmeans_model$tot.withinss = 78869.05) is the best of the three clustering solutions. The clusters in this result are the most compact and well-separated, and the WCSS value is significantly lower than the other two results. This suggests that the K-means algorithm has effectively captured the underlying structure of the data and produced two meaningful clusters.
...
```{r}
over50K=ifelse(dataset$num_salary==2, "Yes", "No")
...
dataset.salary <- subset(dataset, select = -salary)
dataset.salary <- subset(dataset, select = -num_salary)
